...
function finalize_successful_order($order_id, $user_id) {
    ...
    try {
        // Step 1: Lock the order row and verify it's pending for this user.
        // This prevents duplicate processing from webhooks or user actions.
        $sql_verify_order = "SELECT total_amount, status FROM orders WHERE id = ? AND user_id = ? FOR UPDATE";
        $stmt_verify = $mysqli->prepare($sql_verify_order);
        $stmt_verify->bind_param("ii", $order_id, $user_id);
        $stmt_verify->execute();
        $verify_result = $stmt_verify->get_result();
        $order_status_data = $verify_result->fetch_assoc();
        $stmt_verify->close();
        ...
        if ($order_status_data['status'] !== 'Pending') {
            // Order already processed. This is not an error.
            // It's a successful outcome because the order is not stuck in pending.
            $mysqli->commit(); // Release lock
            return ['status' => 'success', 'message' => 'Order already finalized.'];
        }
        ...
        // Step 3: Get order items and their duration
        // Use a LEFT JOIN to handle cases where a product might have been deleted.
        $sql_items = "SELECT oi.product_id, p.name as product_name, p.duration_days
                      FROM order_items oi
        ...
        // Step 4: Create subscriptions for each item
        ...
        $item_names_for_email = [];
        foreach ($items as $item) {
            $product_id = $item['product_id'];
            // Handle lifetime subscriptions (duration <= 0) and potentially missing products (duration is NULL)
            $duration_days = isset($item['duration_days']) ? (int)$item['duration_days'] : 0;
            $item_names_for_email[] = $item['product_name'] ?? '[Deleted Product]';
        ...
        // Step 5: Get user details for the email
        $sql_user = "SELECT username, email FROM users WHERE id = ?";
        $stmt_user = $mysqli->prepare($sql_user);
        $stmt_user->bind_param("i", $user_id);
        $stmt_user->execute();
        $user_details_for_email = $stmt_user->get_result()->fetch_assoc();
        $stmt_user->close();

        // Prepare all data for the email notification
        $email_data = [
            'user' => $user_details_for_email,
            'order_id' => $order_id,
            'total_amount' => $order_status_data['total_amount'],
            'item_names' => $item_names_for_email
        ];
        ...
        $mysqli->rollback();
        error_log("Order finalization failed for Order ID $order_id: " . $e->getMessage());
        return ['status' => 'error', 'message' => 'Failed to finalize order. Please contact support.'];
    }

    // --- Send notification emails AFTER the transaction is safely committed ---
    if ($email_data) {
        try {
            $item_list_html = "<ul><li>" . implode("</li><li>", array_map('htmlspecialchars', $email_data['item_names'])) . "</li></ul>";
            $site_title = get_app_setting('site_title', 'Eflex');

            // 1. Email to the user
            $user_subject = "Your Order Confirmation from " . $site_title;
            $user_body = "
                Hi " . htmlspecialchars($email_data['user']['username']) . ",<br><br>
                ...
                Total Amount: " . format_price($email_data['total_amount']) . "<br>
                Items:<br>
                $item_list_html
                <br>
                ...
                The " . $site_title . " Team
            ";
            send_notification_email($email_data['user']['email'], $user_subject, $user_body);

            // 2. Email to the admin
            $admin_email = get_app_setting('admin_notification_email', get_app_setting('contact_email'));
            if (!empty($admin_email)) {
                $admin_subject = "New Order Notification (#{$email_data['order_id']}) on " . $site_title;
                $admin_body = "
                    A new order has been placed and paid for on your website.<br><br>
                    <b>Order Details:</b><br>
                    Order ID: #{$email_data['order_id']}<br>
                    Customer: " . htmlspecialchars($email_data['user']['username']) . " (" . htmlspecialchars($email_data['user']['email']) . ")<br>
                    Total Amount: " . format_price($email_data['total_amount']) . "<br>
                    Items:<br>
                    $item_list_html
                    <br>
            ...
            error_log("Failed to send order confirmation email for order ID {$email_data['order_id']}: " . $e->getMessage());
        }
    }
