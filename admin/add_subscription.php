...
// Fetch subscription classes for dropdown
...
if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_subscription'])){
    ...
    if(empty($user_id) || empty($product_id)){
        $message = '<div class="alert alert-danger">Please select a user and a class.</div>';
    } else {
        ...
        if($stmt = $mysqli->prepare($sql)){
            ...
            if($stmt->execute()){
                ...
                $message = '<div class="alert alert-danger">Error adding subscription. The user might already be subscribed to this class.</div>';
            }
            $stmt->close();
        }
    }
...
<div class="card">
    ...
    <div class="card-body">
        <p>Manually grant a user access to a subscription class. This is useful for activating subscriptions after a manual payment like a bank transfer.</p>
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
            ...
            <div class="mb-3">
                <label for="product_id" class="form-label">Select Subscription Class</label>
                ...
                <div class="form-text">Only classes that have a Google Drive Folder ID linked to them are shown here.</div>
            </div>
            <div class="mb-3">
